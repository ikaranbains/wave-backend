...
export function setupSocketIO(io) {
  ...
  io.on('connection', (socket) => {
    ...
    // the replay cannot race the app's initial render.
    socket.on('call_sync', () => {
      const pendingCall = [...activeCalls.values()].find(
        (call) =>
          call.status === 'ringing' &&
          call.callerId !== authenticatedUserId &&
          call.participantIds.includes(authenticatedUserId)
      );
      if (pendingCall) socket.emit('incoming_call', incomingCallPayload(pendingCall));
    });
    ...
    socket.on('webrtc_signal', (data) => {
      ...
      const call = activeCalls.get(callId);
      if (!call || !call.participantIds.includes(authenticatedUserId)) return;
      const recipientId = call.participantIds.find((id) => id !== authenticatedUserId);
      if (recipientId) {
        io.to(`user:${recipientId}`).emit('webrtc_signal', {
          callId,
          senderId: authenticatedUserId,
          signal,
        });
